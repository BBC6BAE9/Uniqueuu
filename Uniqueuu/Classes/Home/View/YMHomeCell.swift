...
class YMHomeCell: UITableViewCell {
    ...
    var homeItem: UNHomeItem? {
        didSet {
            let imageURL = homeItem!.cover_image_url!
            bgImageView.kf.setImage(with: ImageResource.init(downloadURL: URL.init(string: imageURL)!), placeholder: nil, options: [KingfisherOptionsInfoItem.transition(ImageTransition.fade(1)), KingfisherOptionsInfoItem.forceRefresh], progressBlock: nil, completionHandler: nil)
