...
protocol UNCollectionViewCellDelegate: NSObjectProtocol {
    func collectionViewCellDidClickedLikeButton(button: UIButton)
}

class UNCollectionViewCell: UICollectionViewCell {
    
    weak var delegate: UNCollectionViewCellDelegate?
    
    var result: YMSearchResult? {
        didSet {
            let url = result!.cover_image_url!
        
            productImageView.kf.setImage(with: ImageResource.init(downloadURL: URL.init(string: url)!), placeholder: nil, options: nil, progressBlock: nil) {
