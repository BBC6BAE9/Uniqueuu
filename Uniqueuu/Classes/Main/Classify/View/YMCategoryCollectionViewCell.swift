...
class YMCategoryCollectionViewCell: UICollectionViewCell {
    ...
    var collection: YMCollection? {
        didSet {
            ...
            collectionImageView.kf.setImage(with: ImageResource.init(downloadURL: URL.init(string: url!)!), placeholder: nil, options: nil, progressBlock: nil) {  (image, error, cacheType, imageURL) in
