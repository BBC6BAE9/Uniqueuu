...
class YMCollectionTableViewCell: UITableViewCell {
    ...
    var collectionPost: YMCollectionPost? {
        didSet {
            ...
            bgImageView.kf.setImage(with: ImageResource.init(downloadURL: URL.init(string: url!)!), placeholder: nil, options: nil, progressBlock: nil) { (image, error, cacheType, imageURL) in
