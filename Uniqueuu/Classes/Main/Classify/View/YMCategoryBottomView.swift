...
class YMCategoryBottomView: UIView {
    ...
    override init(frame: CGRect) {
        ...
        /// 分类界面 风格,品类
        UNNetworkTool.shareNetworkTool.loadCategoryGroup { [weak self] (outGroups) in
    ...
    private func setupButton(index: Int, group: YMGroup) -> YMVerticalButton{
        ...
        button.tag = group.id!
        button.addTarget(self, action: #selector(groupButonClick(button:)), for: .touchUpInside)
        button.titleLabel?.font = UIFont.systemFont(ofSize: 15)
        button.setTitleColor(UNColor(r: 0, g: 0, b: 0, a: 0.6), for: .normal)
        
        
        button.kf.setImage(with: ImageResource.init(downloadURL:NSURL(string: group.icon_url!)! as URL ), for: .normal)
    ...
    private func setupLabel(title: String) -> UILabel {
        ...
        styleLabel.textColor = UNColor(r: 0, g: 0, b: 0, a: 0.6)
