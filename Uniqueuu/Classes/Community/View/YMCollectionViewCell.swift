...
class YMCollectionViewCell: UICollectionViewCell {
    ...
    var result: YMSearchResult? {
        didSet {
            ...
            productImageView.kf.setImage(with: ImageResource.init(downloadURL: URL.init(string: url)!), placeholder: nil, options: nil, progressBlock: nil) {(image, error, cacheType, imageURL) in
